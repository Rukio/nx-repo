...
make build-go-audit-service build-go-athena-service build-go-athena-listener-service
...
(make ensure-docker ensure-dev-kafka ensure-dev-redis && make nobuild-run-go-athena-listener-service) &
wait
