...
const RISK_STRAT_SERVICE_URL = Cypress.env('baseUrl');

function login({ username }: LoginUser) {
  ...
  const password = Cypress.env('userPasswords') as UserPasswordConfig;
    ...
    .then(() => {
      ...
      sendPOSTRequest({
        ...
        return sendPOSTRequest({
          ...
          Cypress.config('baseUrl', RISK_STRAT_SERVICE_URL);
        });
      });
    });
}
