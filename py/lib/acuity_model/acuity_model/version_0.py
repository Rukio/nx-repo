...
class V0(BaseModel):
    """
    V0 is unified interface for the initial acuity model
    The heuristics in this model are transcribed from `acuities.csv`
    There is some transformation needed from the provided csv:
    - The ranges are exclusive on the bottom and inclusive on top
    - The provided acuities are given on a scale of 1-5, which will translate
      to:
       - acuity 1,2 -> return ACUITY_HIGH
       - acuity 3 -> return ACUITY MEDIUM
       - acuity 4-5 -> return ACUITY_LOW
    """

    @property
    def model_namespace(self) -> str:
        return "acuity_model.V0"
